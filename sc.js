...
(function(){
  var vm, fs, path, bootSC, env, dataDir, argv, IsThreaded, Worker, replace$ = ''.replace;
  vm = require('vm');
  fs = require('fs');
  path = require('path');
        ...
        case 'setcrontrigger':
          ...
          return DB.get("cron-nextTriggerTime", function(arg$, nextTriggerTime){
            ...
            if (scheduledNextTriggerTime !== nextTriggerTime) {
              fs.writeFileSync(dataDir + "/nextTriggerTime_debug.txt", nextTriggerTime, 'utf8');
            }
            if (triggerTimeList.length === 0) {
              return DB.hdel("cron-list", room + "_" + timetriggerdata.cell, function(){});
            } else {
