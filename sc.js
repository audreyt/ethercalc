...
(function(){
  var vm, fs, path, FindBin, bootSC, argv, IsThreaded, WorkerJS, self, noop, Boot, Worker, e, replace$ = ''.replace;
  vm = require('vm');
  fs = require('fs');
  path = require('path');
  ...
  IsThreaded = true;
  if (parseInt(replace$.call(process.versions.node, /\..*/, '')) >= 8) {
    WorkerJS = (function(){
      try {
  ...
  if (WorkerJS) {
    ...
    noop = function(){};
    Boot = function(){
      return WorkerThread.parentPort.on('message', function(msg){
        if (msg.eval && msg.port) {
          console.log(msg.eval);
          msg.port.postMessage(eval.apply(this, [msg.eval]));
          msg.port.close();
          return;
        }
    ...
    Worker = (function(){
      ...
      function Worker(code){
        var workerThread, this$ = this;
        this.workerThread = workerThread = new WorkerThread.Worker("global.WorkerThread = require('worker_threads');\nglobal.window = {\n  setTimeout: function(cb) { process.nextTick(cb) },\n  alert: " + noop + ",\n  clearTimeout: " + noop + "\n};\nglobal.self = { onmessage: " + noop + ", thread: {} };\nglobal.alert = " + noop + ";\n(" + Boot + ")();\n(" + code + ")();", {
          eval: true
        });
        workerThread.on('message', function(data){
          return typeof this$.onmessage == 'function' ? this$.onmessage({
            data: data
          }) : void 8;
        });
        this.postMessage = function(data){
          return workerThread.postMessage({
            data: data
          });
        };
        this.thread = {
          ...
          eval: function(src, cb){
            var ref$, port1, port2;
            ref$ = new WorkerThread.MessageChannel(), port1 = ref$.port1, port2 = ref$.port2;
            workerThread.postMessage({
              eval: src,
              port: port1
            }, [port1]);
            return port2.on('message', function(rv){
