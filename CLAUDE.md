...
- `GET /_roomlinks` responds with `Content-Type: text/html` but serializes via `res.json()` — returns JSON array in an HTML content-type response. Preserve.
...
| `execute`       | `{room, cmdstr, user, auth}`                                     | Validates `auth`; rejects `set sheet defaulttextvalueformat text-wiki`; appends log+audit; runs in SC; broadcasts; special submitform flow creates `<room>_formdata` |
| `ask.log`       | `{room, user}`                                                   | Joins `log-<room>` + `user-<user>`; loads snapshot+log+chat; replies `{type:log, room, log, chat, snapshot}`                                                         |
| `ask.recalc`    | `{room}`                                                         | Joins `recalc.<room>`; reloads fresh SC instance; replies `{type:recalc, room, log, snapshot}`                                                                       |
