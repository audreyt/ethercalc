...
Volatile fields (timestamps, socket IDs, snapshot bodies) are normalized before comparison. HTML/XLSX/ODS responses use structural canonicalizers. See `packages/oracle-harness/src/normalize.ts` and `AGENTS.md` §4.2 in the repository.