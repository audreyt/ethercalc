...
describe("roomsFromFilesystem — dump.json blob", () => {
  ...
  it("preserves legacy Sandstorm TOCs whose data is not in sheet1 (#837)", async () => {
    const tocSnapshot = [
      "version:1.5",
      "cell:A1:t:#url",
      "cell:B1:t:#title",
    ...
    const dataSnapshot = ["version:1.5", "cell:A1:t:Android Usage Chart", ""].join("\n");
    const fs = fakeFs({
      "dump.json": JSON.stringify({
        "chat-sheet1": [],
        "snapshot-sheet1": "version:1.5\nsheet:c:1:r:1:tvf:1\n",
        "snapshot-sheet.1": dataSnapshot,
        "snapshot-sheet": tocSnapshot,
        "chat-sheet.1": [],
        "audit-sheet.1": ["set A1 text t Android Usage Chart"],
        "snapshot-sheet.1_formdata": "version:1.5\nsheet:c:1:r:1:tvf:1\n",
      }),
    });
    ...
    expect(byName.get("sheet")?.snapshot).toBe(tocSnapshot);
    expect(byName.get("sheet.1")).toMatchObject({
      snapshot: dataSnapshot,
      audit: ["set A1 text t Android Usage Chart"],
    });
    expect(byName.get("sheet1")?.snapshot).not.toContain("Android Usage Chart");
  });
  ...
  it("ignores non-object / array / null timestamps", async () => {
    const variants = [null, 42, "string", ["an", "array"]];
