...
function buildFullStub(): {
  ...
  const map = new Map<string, { body: string; contentType: string }>([
    ...
    ["/multi/index.html", { body: "<html>multi</html>", contentType: "text/html; charset=utf-8" }],
    ["/multi/assets/index.js", { body: "/* multi */", contentType: "application/octet-stream" }],
    [
      "/multi/assets/index.css",
      { body: "/* multi css */", contentType: "application/octet-stream" },
    ],
...
async function resetDefaultRoom(room: string, commands: readonly string[] = []): Promise<void> {
...
describe("GET / (root)", () => {
  ...
  it("opens legacy Sandstorm data linked from the TOC instead of empty sheet1 (#837)", async () => {
    await resetDefaultRoom("sheet", [
      "set A1 text t #url",
      "set B1 text t #title",
      "set A2 text t /sheet.1",
      "set B2 text t Sheet1",
    ]);
    await resetDefaultRoom("sheet1", ["set A1 empty"]);
    await resetDefaultRoom("sheet.1", ["set A1 text t Android Usage Chart"]);
    ...
    expect(res.status).toBe(302);
    expect(res.headers.get("Location")).toBe("/=sheet");

    const toc = await call("/_/sheet/csv.json");
    expect(await toc.json()).toEqual([
      ["#url", "#title"],
      ["/sheet.1", "Sheet1"],
    ]);
    const data = await call("/_/sheet.1/csv.json");
    expect(await data.json()).toEqual([["Android Usage Chart"]]);
  });
...
describe("GET /multi/*", () => {
  it("serves the React workbook bundle with standalone-workerd MIME types", async () => {
    const stub = buildFullStub();
    const js = await call("/multi/assets/index.js", { ASSETS: stub.ASSETS });
    expect(js.status).toBe(200);
    expect(js.headers.get("Content-Type")).toBe("application/javascript; charset=utf-8");
    expect(await js.text()).toBe("/* multi */");

    const css = await call("/multi/assets/index.css", { ASSETS: stub.ASSETS });
    expect(css.status).toBe(200);
    expect(css.headers.get("Content-Type")).toBe("text/css; charset=utf-8");
    expect(await css.text()).toBe("/* multi css */");
    expect(stub.calls.slice(-2).map((call) => call.pathname)).toEqual([
