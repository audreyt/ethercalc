...
function makeStorage(state: StorageState): WsStorage {
  ...
  return {
    async listPrefix(prefix: string): Promise<string[]> {
      if (prefix === "log:") {
        return Array.from(state.log.keys())
          .sort()
          .map((k) => state.log.get(k)!);
      }
      if (prefix === "chat:") {
        return Array.from(state.chat.keys())
          .sort()
          .map((k) => state.chat.get(k)!);
      }
      return [];
    },
...
function makeCtx(opts: MakeCtxOpts = {}): { ctx: WsContext; calls: CallLog; state: StorageState } {
  ...
  const ctx: WsContext = {
    ...
    async applyCommand(cmdstr: string): Promise<boolean> {
      ...
      state.log.set(`log:${String(state.log.size).padStart(10, "0")}`, cmdstr);
      state.snapshot = "SNAP";
      return true;
    },
    ...
    async allowSubmitForm() {
      return opts.allowSubmitFormOk !== false;
    },
...
describe("handleExecute", () => {
  ...
  it("submitform forwards sibling commands and broadcasts with includeSelf=true", async () => {
    ...
    expect(calls.siblingFetches[0]!.init?.body).toBe("set A1 value n 1\rset A2 value n 2");
    expect(calls.broadcasts).toHaveLength(1);
  ...
  it("submitform returns silently when allowSubmitForm is false", async () => {
    const { ctx, calls } = makeCtx({ allowSubmitFormOk: false });
    await handleExecute(ctx, {
      type: "execute",
      room: "r",
      user: "u",
...
describe("dispatchWsMessage end-to-end interactions", () => {
  ...
  it("stopHuddle wipes after prior chat/log entries", async () => {
    ...
    expect(calls.broadcasts.some((b) => b.msg.type === "stopHuddle")).toBe(true);
  });
});
