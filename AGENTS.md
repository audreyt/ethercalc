...
| 14 | Passkeys (Phase A): singleton `AuthDO` is the WebAuthn RP (`ETHERCALC_AUTH` + RP_ID/RP_NAME/ORIGIN trust anchors from env only); RoomDO is the SOLE authz boundary via local `meta:access`/`meta:acl` for top-level rooms or immutable `meta:parent` delegation for workbook children (Worker cookie→`X-EC-Uid` is UX only, `doFetch` strips inbound identity/capability spoofs); private rooms use deny-overrides, including parent-marker/local-ACL conflicts (legacy `?auth=` demoted); `/_do/ping` + `/_do/pitr-*` are gate-exempt operator paths; `ETHERCALC_KEY` never signs sessions |
