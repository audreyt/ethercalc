...
| **Durable Objects** (`RoomDO` / `AuthDO`) | **NO BULK EXPORT API EXISTS** in Cloudflare Workers for Durable Object SQLite storage.<br><br>**D1 Mirror Scope & Reality**: D1 `ethercalc_rooms` holds **ONLY** the cross-room index (`room`, `updated_at`, `cors_public`) per `packages/worker/migrations/0001_rooms.sql:17-21` and `packages/worker/src/lib/rooms-index.ts:46-49`. **NO SocialCalc sheet snapshots, command logs, cell values, chat messages, or audit history are stored in D1.**<br><br>**Available Safeguards:**<br>1. Per-room PITR snapshot restoration via operator endpoint `POST /_/:room/pitr-restore` (requires `ETHERCALC_MIGRATE_TOKEN`, §0.1).<br>2. Cloudflare platform-level Durable Object storage replication and durability. | In-DO SQLite storage (`state.storage`) and per-room PITR history buffers. *(D1 export recovers ONLY room names and timestamps, NOT sheet content).* | **NO OPERATOR BULK BACKUP**: RoomDO sheet content (snapshots, logs, cells) cannot be exported in bulk. A `wrangler d1 export` backs up room index metadata (`room`, `updated_at`, `cors_public`), NOT sheet cell state. `AuthDO` credentials (passkeys, session revocation lists) and un-saved in-memory WebSocket operational state also CANNOT be exported. Recovery of sheet content relies entirely on per-room PITR or Cloudflare platform storage durability. |
...
1. **What `wrangler d1 export` Backs Up**:
   - Backs up the `rooms` index table (`room`, `updated_at`, `cors_public`).
   - Allows restoring the list of room names and their last-modified timestamps.
   - **DOES NOT** back up or restore sheet cell data, SocialCalc command logs, snapshots, chat history, or audit logs.

2. **What Cloudflare D1 Time Travel Restores**:
   - Restores the `rooms` index table to a prior point-in-time (§6.4).
...
> **CRITICAL OPERATOR NOTICE**: Restoring D1 via Time Travel or SQL dump restores ONLY the cross-room index (`rooms` table containing room names, updated_at timestamps, and cors_public flags per `packages/worker/migrations/0001_rooms.sql:17-21`). It **DOES NOT** roll back or restore Durable Object SQLite storage (`RoomDO` sheet cell data, SocialCalc snapshots, or command logs). Because DO sheet content has no operator bulk export API, cutover safety relies on the three-phase rollout sequence (§4) and per-room PITR recovery.
