...
8. **Mutation-ratchet fragility during the cutover window** (all six audited packages)
   ...
   - **Why they differ (~0.58 pp):** not different file sets (`git diff 327fa3d..09673a4` under worker mutate globs is empty; `robots.ts` already at `327fa3d`), not covered-vs-total gating, and not the `socialcalc-308` reverse-compat alias (that alias is only referenced from `packages/socialcalc-headless/test/legacy-snapshot-reverse.node.test.ts`; worker mutation loads `@ethercalc/socialcalc-headless` via production paths that use checked-in `socialcalc.bundled.ts`, and does not run the headless reverse-compat test). Retained aggregates only: CI−local **Killed +35 / Timeout +5 / Survived −35 / NoCoverage 0 / denom +5**, plus local `RuntimeError` 5 vs CI `# errors` 0. Without CI `mutation.json`, per-mutant transitions are unproven — label the gap **environment-dependent status classification** between the two runs, not a reconstructed flapping story. Same contract, **not** a byte-identical score predictor run-to-run. Detail: [`docs/migration/MUTATION_SURVIVORS.md`](./MUTATION_SURVIVORS.md).
   ...
   - **Operational consequence**: every audited package still sits within one percentage point of its floor on the **local** sample, and worker's local margin is only three hundredths — so another environment-dependent reclassification near the floor can fail a run even when a prior run passed. **Do not** read CI's +0.61 pp on this PR as "comfortable forever": the enforced metric is the same total score, and the next CI runner can land closer to the local sample. A pre-cutover PR that touches these packages — **especially `@ethercalc/worker`** — can still trip `mutation-gate` when functional tests pass. Run `scripts/ratchet-verify.sh` locally *before* pushing as a regression smoke; treat CI's printed total as the enforced result for that head, not as proof the local scarier margin was wrong.
