...
| `CURRENT_PROD_ANNOTATION` (git SHA / message if present) | Same time | **None present** on the live version. Working candidate window remains `[d2afa90, b7d8840)` **if** the live upload used source `wrangler.toml`. Live markers (`compatibility_date 2024-11-12`; passkey UI + appcache; `index-bootstrap.js` 404) match pre-`b7d8840` *source* config but are **not exclusive** under the §4.0 generated-config redirect (which forces compat `2024-11-12` and drops the five extracted scripts from the asset tree). |
...
| `SHIP_VERSION_ID` | Output of `versions upload` + ramp | **`0e024a32-5ce8-4315-a4eb-a4c5c2a8f780`** uploaded 2026-08-10T12:22:35Z; bindings prod (`ethercalc_rooms`, `AUTH="1"`, `RP_ID=ethercalc.net`). Timeline UTC: **12:22:44** CURRENT@100%/SHIP@0% → override smoke PASS → **12:24:35** @10% → **12:25:56** @50% → **12:27:58** **SHIP@100%**. Default traffic now ship only. Rollback floor remains `bd76bda5-3161-4576-b159-dbdb97d774c2`. |
| Edge purge confirmation | After traffic-affecting HTML/JS steps | **NEEDED (operator Dashboard).** At SHIP@100%, uncached probes PASS (`robots.txt?v=…` → `text/plain; charset=utf-8` `User-agent: *`; `index-bootstrap.js?v=…` → **200**; unique-room `X-Robots-Tag: noindex, nofollow, noarchive`; whoami `enabled:true`). Bare `/robots.txt` and bare `/static/index-bootstrap.js` can still show **cf-cache-status: HIT** stale pre-ship HTML/404 until purge. **Cloudflare Dashboard → Caching → Configuration → Purge Everything** for `ethercalc.net` (and `www` if applicable). |
| Post-ramp probes (no override) | After @100% | health ok; whoami `enabled:true`; uncached robots plain + Allow:/ no Disallow; uncached bootstrap 200; `/` 200 references `static/index-bootstrap.js` (cached HTML may lag until purge); unique room X-Robots present; `/` no X-Robots on uncached fetch. **No `enabled:false` at any ramp step.** |
...
6. **`compatibility_date` source bump 2024-11-12 → 2026-07-21** lands in **`b7d8840`** (`security: harden EtherCalc trust boundaries`); parent still has `2024-11-12`. Live prod *reports* `2024-11-12`, but §4.0’s generated `wrangler.json` also forces that older date — so the live report alone does not prove the ship still needs the bump. Staging rehearsal must use source `wrangler.toml` (ship date `2026-07-21`) and the §4.0 no-redirect check **`[OPERATOR-VERIFY]`**.
7. **Candidate-range / exact SHA still open** — version ID is pinned (`bd76bda5…`); exact git SHA is unknown (no annotation). Live markers are consistent with pre-`b7d8840` under source config, but a redirected `b7d8840`+ upload remains possible (§4.0). Strategy shape (single ramp, AUTH stays on) is stable across the window; **changelog/comms and exact probe expectations** may still depend on which intermediate fixes are already live.
...
8. **Mutation-ratchet fragility during the cutover window** (all six audited packages)
   - **What the gate enforces**: each package's `stryker.conf.json` `thresholds.break` is compared to Stryker's **total** `mutationScore` = `(Killed+Timeout)/(Killed+Timeout+Survived+NoCoverage)`. The clear-text **covered** column is display-only. CI job `mutation-gate` runs the same `vp run … mutation` as local `scripts/ratchet-verify.sh` on packages with `packages/*/src/` changes; exit fails iff total score `< break`. For `@ethercalc/worker`, **`break` is 90**. Verified from Stryker source (`mutation-test-report-helper.ts` `determineExitCode`).
   - **Retained `@ethercalc/worker` samples (same config; local vs two CI runs):**

     | Source | Tip | Total score (gated) | Covered (display) | Killed | Timeout | Survived | No cov | Errors | Denom | Margin vs 90 | Provenance |
     | :--- | :--- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | :--- |
     | Local `scripts/ratchet-verify.sh` | `327fa3d` | **90.03%** | (not emphasized) | 5488 | 41 | **570** | 42 | 5 RuntimeError (excl. from denom) | **6141** | **+0.03 pp** | on-disk `reports/mutation/mutation.json` mtime 2026-08-10; full six-package ratchet ~828s |
     | CI `mutation-gate` | `09673a4` | **90.61%** | 91.24% | **5523** | **46** | **535** | **42** | 0 | **6146** | **+0.61 pp** | run [`31390939451`](https://github.com/audreyt/ethercalc/actions/runs/31390939451) job `93462677720`, job wall 20m36s, 2026-08-10 |
     | CI `mutation-gate` | `c31d88c` | **90.61%** | 91.24% | **5523** | **46** | **535** | **42** | 0 | **6146** | **+0.61 pp** | run [`31393353474`](https://github.com/audreyt/ethercalc/actions/runs/31393353474) job `93470615147`, job wall 21m30s / Stryker 21m10s, 2026-08-10 |

   - **CI source identity across the two green runs:** `git rev-parse 09673a4:packages` = `git rev-parse c31d88c:packages` = `e22410dc5a328b81c936a17447b46905da435db1`. Diff between those heads is documentation-only under `docs/migration/` — no `packages/` or `scripts/` paths. On that byte-identical mutated tree the two CI runs **repeated** score 90.61 and counts 5523/46/535/42. That is observed repeatability for those two samples; it is **not** a general determinism claim, and it does not decide which environment is correct.
   - **Local vs CI discrepancy — observed, cause unresolved:** CI−local aggregates are Killed **+35** / Timeout **+5** / Survived **−35** / NoCoverage 0 / denom **+5** (6146 vs 6141), total **+0.58 pp**. Ruled out as explanations: different mutate file sets (`git diff` under worker mutate globs empty from `327fa3d` through both CI heads; `robots.ts` already at `327fa3d`), covered-vs-total gating confusion, and the `socialcalc-308` reverse-compat alias (only referenced from `packages/socialcalc-headless/test/legacy-snapshot-reverse.node.test.ts`; worker mutation uses checked-in `socialcalc.bundled.ts` and does not run that test). **Untested hypotheses** for the remaining gap (not findings): (1) local's 5 `RuntimeError` mutants are excluded from the denominator while CI reported 0 errors and a denominator larger by 5 — whether they are the same mutants is unknown without a per-mutant diff; (2) timing-sensitive Timeout/Killed/Survived classification on different hardware or load. **What would settle it:** the same Stryker config run in both environments with both `mutation.json` artifacts retained and per-mutant statuses diffed. Until then, do not label the gap "run-to-run nondeterminism," do not infer CI determinism from two matching samples, and do not declare either side authoritative. Detail: [`docs/migration/MUTATION_SURVIVORS.md`](./MUTATION_SURVIVORS.md).
   ...
   - **Operational consequence**: every audited package still sits within one percentage point of its floor on the **local** sample, and worker's local margin is only three hundredths. Across both retained CI samples the enforced margin was **+0.61 pp** (90.61 vs 90) — that is the observed CI margin, not a permanent comfort band. A pre-cutover PR that touches these packages — **especially `@ethercalc/worker`** — can still trip `mutation-gate` when functional tests pass. Run `scripts/ratchet-verify.sh` locally *before* pushing as a regression smoke; treat each CI printed total as the enforced result for that head only.
