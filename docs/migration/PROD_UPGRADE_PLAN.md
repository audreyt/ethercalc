...
(Old root HTML references inline bootstraps + `manifest.appcache` and does not fetch `static/index-bootstrap.js`; mixed edge HTML/JS is the dominant asset-skew failure mode for this delta — §C.5.)
...
1. **Session cookie rename (`ec_sess` → `__Host-ec_sess`)** — security-audit change in `packages/worker/src/lib/session.ts`. New Worker ignores the old cookie name; users with only `ec_sess` appear signed out until they passkey-login again (new cookie issued). During percentage ramp, clients may alternate versions and see intermittent whoami/uid. **Mitigations:** prefer low-traffic window; communicate “sign in again if private sheets 403”; consider staging a dual-read compatibility window in code only if operator rejects re-login (**not** in-repo today — would be a pre-cutover PR). Rollback to `CURRENT_PROD_VERSION_ID` restores old cookie name behavior for browsers that still hold `ec_sess`.
2. **Root asset layout skew** — production root still has inline bootstraps + `manifest.appcache`; ship root uses `static/index-bootstrap.js` (and related page scripts) and drops appcache (`b7d8840`). Edge or browser holding old HTML breaks boot if partial assets purge. **Mitigations:** mandatory purge at first traffic shift and at 100%; Probe root + `index-bootstrap.js`; §4.5–§4.6 / `SKEW_AND_RECONNECT.md` still apply for open tabs.
...
4. **Security-audit hardening behavior** — frame/chat/cell caps, per-socket rates, body limits, export sanitizer tighten, CSP/`connect-src` origin anchoring, `__Host` cookie, AuthDO revoke-on-logout + per-IP ceremony limits, `/_timetrigger` gate, etc. Expect more 413/1008/401 responses on abusive or oversized clients; public anonymous write to public rooms remains by design.
5. **Open-tab WS reconnect without re-hydrate** — unchanged client limitation (`hadSnapshot`); §4.5–§4.6 still apply. Form/app-mode tabs may need reload.
...
- §4.5–§4.6 skew / reconnect analysis (+ companion `SKEW_AND_RECONNECT.md`)  
