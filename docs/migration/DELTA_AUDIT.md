...
| **j) SocialCalc Upgrade & Serialisation** | `[FORWARD-COMPATIBLE]` | Low (Bidirectional: 3.1.0 reads pre-3.1.0 oracle saves; genuine SocialCalc **3.0.8** reads 3.1.0-written saves including `part:edit`/`part:audit`. First re-save is semantic, not byte-identical. Note: Phase 1/`149ebcf` already ships the same 3.1.0 bundle as `main`.) | Covered by `legacy-snapshot-compat.test.ts` + `legacy-snapshot-reverse.node.test.ts`. |
